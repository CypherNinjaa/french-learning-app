-- ====================================================================
-- COMPLETE DATABASE SCHEMA FOR FRENCH LEARNING APP
-- Production-ready schema with all tables, relationships, triggers, 
-- RLS policies, and default data
-- ====================================================================

-- Drop existing tables in dependency order (for clean restart)
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, username, full_name, avatar_url, level, points, streak_days)
  VALUES (
    NEW.id,
...
RETURNS INTEGER AS $$
BEGIN
  -- Level formula: 1000 * (N - 1)² + 500 * (N - 1)
...
-- ================================================
-- 10. USER SESSIONS AND ANALYTICS
-- ================================================
...
-- ================================================
-- 11. CONTENT API LAYER (Stage 3.3)
-- ================================================
...
CREATE INDEX IF NOT EXISTS idx_vocabulary_word_type ON vocabulary(word_type);
CREATE INDEX IF NOT EXISTS idx_vocabulary_active ON vocabulary(is_active);

CREATE INDEX IF NOT EXISTS idx_grammar_difficulty ON grammar_rules(difficulty_level);
CREATE INDEX IF NOT EXISTS idx_grammar_category ON grammar_rules(category);
CREATE INDEX IF NOT EXISTS idx_grammar_active ON grammar_rules(is_active);
...
CREATE INDEX IF NOT EXISTS idx_lesson_vocabulary_vocab_id ON lesson_vocabulary(vocabulary_id);
CREATE INDEX IF NOT EXISTS idx_lesson_grammar_lesson_id ON lesson_grammar(lesson_id);
CREATE INDEX IF NOT EXISTS idx_lesson_grammar_rule_id ON lesson_grammar(grammar_rule_id);
...
BEGIN
    ...
    CASE 
        ...
        WHEN total_points < 1100 THEN RETURN 4;
        WHEN total_points < 1850 THEN RETURN 5;
        WHEN total_points < 2850 THEN RETURN 6;
        ELSE RETURN 7;
    END CASE;
END;
$$ language 'plpgsql';
...
CREATE POLICY "Users can access own user achievements" ON user_achievements
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Allow public read access to daily challenges" ON daily_challenges 
    FOR SELECT USING (true);

CREATE POLICY "Users can access own challenge completions" ON user_challenge_completions
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Allow public read access to leaderboards" ON leaderboard_entries 
    FOR SELECT USING (true);

CREATE POLICY "Users can access own streak shields" ON streak_shields
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users can access own gamification stats" ON user_gamification_stats
    FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Users can access own streaks" ON daily_streaks
    FOR ALL USING (auth.uid() = user_id);
...
('Pronunciation', '#FD79A8', 'Pronunciation practice')
ON CONFLICT (name) DO NOTHING;
...
ON CONFLICT (user_role, permission_name) DO NOTHING;

-- ================================================
-- 19. CREATE AVATAR STORAGE BUCKET AND POLICIES
-- ================================================
...
SELECT 'French Learning App Database Schema Created Successfully! 🎉' as message,
...
UNION ALL
SELECT 'milestone_rewards', COUNT(*) FROM milestone_rewards
UNION ALL
